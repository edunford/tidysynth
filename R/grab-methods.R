...
#' grab_unit_weights
#'
#' Grab the unit weights generated by `generate_weights()`from the `synth_tbl`.
#'
...
#' @export
grab_unit_weights.synth_tbl <- function(data,placebo=F){
...
#' grab_predictors
#'
#' Grab the aggregate-level covariates generated by `generate_predictor()` from
#' the `synth_tbl`.
#'
#' @param data nested data of type `synth_tbl`
...
#' grab_outcome
#'
#' Grab the outcome variable from the `synth_tbl`.
#'
#' @param data nested data of type `synth_tbl`
#' @param type string specifying which version of the data to extract: "treated"
#'   or "control". Default is "treated".
...
#' @export
grab_outcome.synth_tbl <- function(data,type="treated",placebo=F){
...
#'
#' Grab the predictor variable weights generated by `generate_weights()`from the
#' `synth_tbl`.
#'
#' @param data nested data of type `synth_tbl`
#' @param placebo boolean flag; if TRUE placebo values are returned as well (if
...
#' @export
grab_predictor_weights.synth_tbl <- function(data,placebo=F){
...
#' grab_loss
#'
#' Grab the weights RMSE loss from the optimization task from the `synth_tbl`.
#'
#' @param data nested data of type `synth_tbl`
#'
#' @return tibble data frame
...
#' @export
grab_loss.synth_tbl <- function(data){
...
#'
#' Grab the synthetic  control generated using `generate_control()`from the
#' `synth_tbl`.
#'
#' @param data nested data of type `synth_tbl`
#' @param placebo boolean flag; if TRUE placebo values are returned as well (if
...
#' @export
grab_synthetic_control.synth_tbl <- function(data,placebo=F){
...
#'
#'   - `pre_mspe`: pre-intervention period means squared precise error
#'
#'   - `post_mspe`: post-intervention period means squared precise error
#'
...
#' @export
grab_signficance.synth_tbl <- function(data,time_window = NULL){
  ...
  data %>%
    ...
    dplyr::summarize(.placebo = mean(.placebo),
                     mspe = sum((real_y - synth_y)^2)/dplyr::n() ) %>%
    ...
    dplyr::mutate(rank = dplyr::row_number(),
                  ...
                  z_score = (mspe_ratio-mean(mspe_ratio))/sd(mspe_ratio),
...
#' intervnetion unit, the synthetic control, and the donor pool average. Table
...
#'   generate_control()
#'
#' # Compare the distributions between the observed data, the synthetic control, and the donor pool average
...
#' @export
grab_balance_table.synth_tbl <- function(data){
