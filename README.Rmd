...
In the current implementation, you cannot unpack an entire `synth_tbl` using `unnest()`. This is due to disparities in the dimensions between the donor and treated units. The `grab_` function is meant to streamline any specific extraction needs. Alternatively, once can unnest individual columns as per usual, but must first convert the `synth_tbl` to a `tibble` data type.
